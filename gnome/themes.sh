...
if ! dpkg -l papirus-icon-theme &>/dev/null 2>&1; then
  ...
  ok "Papirus já instalado"
fi

# Kanagawa GTK theme
REPO="https://github.com/Fausto-Korpsvart/Kanagawa-GKT-Theme.git"
TMP=$(mktemp -d)

if ls "$THEMES_DIR"/Kanagawa* &>/dev/null 2>&1; then
  ok "Kanagawa GTK theme já instalado"
else
  log "Instalando Kanagawa GTK theme..."
  if git clone --depth=1 "$REPO" "$TMP/kanagawa" 2>/dev/null; then
    if [[ -d "$TMP/kanagawa/themes" ]]; then
      cp -r "$TMP/kanagawa/themes/"* "$THEMES_DIR/"
      ok "Kanagawa GTK instalado em $THEMES_DIR"
    else
      warn "Estrutura do repositório mudou — instale manualmente de $REPO"
    fi
  else
...
# Cursor theme (Bibata Modern Ice — neutro e bonito)
if ! fc-list | grep -qi "Bibata" && ! ls "$ICONS_DIR"/Bibata* &>/dev/null 2>&1; then
  ...
  curl -fsSL \
    ...
    -o "$TMP2/cursor.tar.xz" 2>/dev/null && \
    tar -xf "$TMP2/cursor.tar.xz" -C "$ICONS_DIR" && \
    ok "Cursor Bibata Modern Ice instalado" || \
    warn "Não foi possível baixar o cursor"
  rm -rf "$TMP2"
else
  ok "Cursor já configurado"
fi
